...
const flw = new Flutterwave("FLWPUBK-9f43b7017c6e99864c5130cca45b7baa-X", process.env.FLW);
...
async function validateAccount(account_number, bank_code) {
    ...
    await fetch("https://api.flutterwave.com/v3/accounts/resolve", {
        headers: {
            "Authorization": `Bearer ${process.env.FLW}`,
...
async function LoadAirtime(number, amount) {
    let ref = await generateID();
    let res;
    const payload = {
        country: 'NG',
        customer: number,
...
const initTrans = async (account_number, narration, amount, bank_code) => {
    ...
    await fetch("https://api.flutterwave.com/v3/transfers", {
        ...
        .catch(error => {
            res = {
                ...
                data: {
                    id: 71054208,
                    account_number: '2100201581',
                    bank_code: '033',
                    full_name: 'OWOEYE OMONIYI SAMSON',
                    created_at: '2024-03-12T10:54:30.000Z',
                    currency: 'NGN',
                    debit_currency: 'NGN',
                    amount: 100,
                    fee: 10.75,
                    status: 'NEW',
                    reference: 'transfer-1710240869730',
                    meta: null,
                    narration: 'Omoniyi Owoeye | ',
                    complete_message: '',
                    requires_approval: 0,
                    is_approved: 1,
...
const checkBalance = async () => {
    try {
        ...
        const ngn = response.data.filter(ex => ex.currency === "NGN");
        return ngn;
    } catch (error) {
        return [{ currency: 'NGN', available_balance: 0.00, ledger_balance: 0 }]
    }
}

module.exports = { flw, getBanks, validateAccount, initTrans, checkTrans, checkBalance, LoadAirtime };